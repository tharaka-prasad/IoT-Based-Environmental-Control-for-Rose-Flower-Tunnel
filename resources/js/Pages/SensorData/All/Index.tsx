...
const SensorDataIndex: React.FC<SensorDataProps> = ({ auth, sensors = [] }) => {
    ...
    return (
        ...
        >
                                ...
                                <table className="min-w-full bg-white border border-gray-200">
                                    <thead>
                                        <tr className="bg-gray-100">
                                            ...
                                            <th className="px-4 py-2 text-left">LDR (%)</th>
                                        ...
                                        {thingSpeakData.length > 0 ? (
                                            thingSpeakData.map((feed: Sensor, index: number) => (
                                                <tr key={index} className="border-t">
                                                    ...
                                                    <td className="px-4 py-2">{feed.humidity}</td>
                                                    <td className="px-4 py-2">{feed.soil_moisture}</td>
                                                    <td className="px-4 py-2">{feed.gas}</td>
                                                    <td className="px-4 py-2">{feed.ldr}</td>
            ...
            {/* <SensorDataViewModal isOpen={isViewModalOpen} onClose={closeViewModal} sensor={selectedSensor} /> */}
        </AuthenticatedLayout>
    );
};
